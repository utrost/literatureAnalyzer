...
1. **Extract the shared schema into a package.** ✅ *shipped.* The four contract types (`Shape`/`StyleProfile`/`WorldSeed`/`BeatPlan` + parts) now live once in the **`story_contract`** package — the analyzer repo's `./contract` subdirectory (decision: the package lives inside the analyzer). Both tools import from it: the analyzer via a local path source, Endless via a `uv` git-subdirectory dependency; each `schemas.py` re-exports. The two verbatim copies that could silently drift are gone. A **contract drift guard** (`tests/test_contract.py` + a byte-identical `contract.schema.json` in both repos) additionally catches any unintended structural change to the contract. To pull a later contract change into Endless: `uv lock --upgrade-package story-contract`.
2. **Build the headless "edit core."** ✅ *shipped* (`lit_analyzer/edits.py`): `apply_edits(analysis, ["path=value", …])` mutates contract fields by dotted path (lists index by element id; `|` splits a list value), re-validates against the schema (a bad value is rejected, not shipped), and flows into `--emit-endless`/`--transpose`/`--compare`; `validate_contract()` lints cross-field invariants (bad `protagonist_id`, duplicate/dangling ids). Exposed as `deconstruct --set … [--validate]`, so there's a **non-GUI CLI editing path** today. Deterministic and fully tested — the Workbench GUI becomes a thin front-end over this.

**Design decisions (made):**
- **Schema package location:** ✅ inside the analyzer repo (`./contract`, the `story_contract` package), consumed by Endless via a git-subdirectory dependency. Shipped (prerequisite 1).
- **Workbench GUI stack:** **Gradio** — schema-driven forms over the Pydantic contract types, runs on localhost, minimal deps; matches the single-user local-first ethos. (A full FastAPI+React app or a TUI were the alternatives.)
- **Workbench repo:** still open — a new sibling repo (keeps a stateful, heavier-dep GUI out of the two clean CLI/library repos — the leaning) vs. a package inside `literatureAnalyzer`. Whichever, it builds on `story_contract` + `lit_analyzer.edits` (both shipped).
